...
function ChosenMoviesGenreView({ genreId }) {
  ...
  useEffect(() => {
    async function fetchData() {
      const request = await axios.get(
        `/discover/movie?api_key=${process.env.REACT_APP_MY_API_KEY}&with_genres=${genreId.genreId}`
      );
  ...
  return (
    <div className="moviesGenreView">
      {movies.map((tempMovie) => (
        <Link to={{ pathname: `/movies/${tempMovie.id}` }}>
